...
CLASS_NAMES = {
    0: "BACKGROUND",
    1: "Epithelial",  # (Normal + Non-Neoplastic + Myoepithelial)
    2: "Tumor Epithelial",  # (Tumor + Invasive Carcinomatous + Non-Neoplastic)
    3: "Lymphocytes",
    4: "Macrophages",
    5: "Neutrophils",
    6: "Plasma",
    7: "Eosinophils",
    8: "Necrotic",  # (Dead Included)
    9: "Muscle",
    10: "Fibroblasts",
    11: "Endothelial",
    12: "Adipocytes",
...
# 5: "Eosinophil",
# 6: "Connective",
# 7: "Macrophage",# remove for lizard patch level ds
...
def cls_dict_from_label(y, y_class):
    return dict(
        (r.label, int(np.median(y_class[r.slice][y[r.slice] == r.label])))
...
def oversample_classes(X, Y, D, Y0, idx, n_extra_classes=4, seed=None):
    ...
    # extra_classes = np.argsort(class_counts)[:0]
    # print(class_counts)
    # print(extra_classes)

    # Filter out zero elements and perform sorting
    non_zero_indices = np.nonzero(class_counts)  # Get the indices of non-zero elements
    sorted_indices = np.argsort(
        class_counts[non_zero_indices]
    )  # Sort the non-zero elements
    extra_classes = non_zero_indices[0][sorted_indices][
        :n_extra_classes
    ]  # Select the first n_extra_classes indices
...
def augment_stains(
    x, amount_matrix=0.2, amount_stains=0.2, n_samples=1, subsample=128, rng=None
):
    """
...
class HEStaining(BaseTransform):
    ...
    def __init__(self, amount_matrix=0.15, amount_stains=0.4):
        super().__init__(
            default_kwargs=dict(
                amount_matrix=amount_matrix, amount_stains=amount_stains
            ),
...
def refine(labels, polys, thr=0.5, w_winner=2, progress=False):
    ...
    for k, i in tqdm(
        ...
        polys_i_suppressed = nms["coord"][
            nms["suppressed"] == i
        ]  # suppressed polys by winner
...
def predict(
    ...
    **kwargs,
    ...
    if not all(
        [m.keras_model.output_shape == model[0].keras_model.output_shape for m in model]
    ):
        raise ValueError(
    ...
    kwargs_instances.setdefault(
        "nms_thresh", np.mean([m.thresholds.nms for m in model])
    )
    kwargs_instances.setdefault(
        "prob_thresh", np.mean([m.thresholds.prob for m in model])
    )
    ...
    warnings.warn(
        f"combining {len(model)} models -> total {len(model_augs)} predictions"
    )
    ...
    prob_class = [
        rot90(flip(v, f, False), -k, False)
        for (m, (k, f)), v in zip(model_augs, prob_class)
    ]
    ...
    class_count = count_classes(
        out, classes=range(1, 7), crop=(224, 224) if crop_counts else None
    )
