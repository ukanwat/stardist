...
def group_matching_labels(ys, thresh=1e-10, criterion="iou"):
    ...
    if isinstance(ys, np.ndarray):
        ...
        all(_check_label_array(y, "ys") for y in ys) or _raise(
            ValueError("'ys' must be a list of label images")
        )
        all(y.shape == ys[0].shape for y in ys) or _raise(
            ValueError("all label images must have the same shape")
        )
    ...
    def _match_single(y_prev, y, next_id):
        ...
        res = matching(
            y_prev, y, report_matches=True, thresh=thresh, criterion=criterion
        )
